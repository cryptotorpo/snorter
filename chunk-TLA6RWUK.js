  ...
  k = class extends c {
    constructor({ param: t, type: r, modifier: o }) {
      super("Invalid ABI parameter.", {
        details: t,
        metaMessages: [
          `Modifier "${o}" not allowed${r ? ` in "${r}" type` : ""}.`,
        ],
      }),
        Object.defineProperty(this, "name", {
          enumerable: !0,
...
function ue(e, t, r) {
  ...
  if (r)
    for (let n of Object.entries(r)) {
      if (!n) continue;
      let i = "";
      for (let a of n[1]) i += `[${a.type}${a.name ? `:${a.name}` : ""}]`;
      o += `(${n[0]}{${i}})`;
    }
  return t ? `${t}:${e}${o}` : e;
}
...
function ce(e, t = {}) {
  if (G(e)) return ye(e, t);
  if (W(e)) return be(e, t);
  if (D(e)) return xe(e, t);
  if (ee(e)) return ge(e, t);
  if (ne(e)) return he(e);
  if (ae(e)) return { type: "receive", stateMutability: "payable" };
  throw new T({ signature: e });
}
function ye(e, t = {}) {
  let r = H(e);
  if (!r) throw new d({ signature: e, type: "function" });
  let o = f(r.parameters),
    n = [],
    i = o.length;
  for (let s = 0; s < i; s++)
    n.push(b(o[s], { modifiers: P, structs: t, type: "function" }));
  let a = [];
  if (r.returns) {
    let s = f(r.returns),
      u = s.length;
    for (let m = 0; m < u; m++)
      a.push(b(s[m], { modifiers: P, structs: t, type: "function" }));
  }
  return {
    name: r.name,
    type: "function",
...
function be(e, t = {}) {
  let r = V(e);
  if (!r) throw new d({ signature: e, type: "event" });
  let o = f(r.parameters),
    n = [],
    i = o.length;
  for (let a = 0; a < i; a++)
    n.push(b(o[a], { modifiers: ie, structs: t, type: "event" }));
  return { name: r.name, type: "event", inputs: n };
}
function xe(e, t = {}) {
  let r = N(e);
  if (!r) throw new d({ signature: e, type: "error" });
  let o = f(r.parameters),
    n = [],
    i = o.length;
  for (let a = 0; a < i; a++) n.push(b(o[a], { structs: t, type: "error" }));
  return { name: r.name, type: "error", inputs: n };
}
function ge(e, t = {}) {
  let r = te(e);
  if (!r) throw new d({ signature: e, type: "constructor" });
  let o = f(r.parameters),
    n = [],
    i = o.length;
  for (let a = 0; a < i; a++)
    n.push(b(o[a], { structs: t, type: "constructor" }));
  return {
    type: "constructor",
    stateMutability: r.stateMutability ?? "nonpayable",
    inputs: n,
...
function he(e) {
  let t = oe(e);
  if (!t) throw new d({ signature: e, type: "fallback" });
  return {
    type: "fallback",
...
function b(e, t) {
  ...
  let o = w.test(e),
    n = l(o ? $e : ve, e);
  if (!n) throw new M({ param: e });
  if (n.name && Pe(n.name)) throw new I({ param: e, name: n.name });
  let i = n.name ? { name: n.name } : {},
    a = n.modifier === "indexed" ? { indexed: !0 } : {},
    s = t?.structs ?? {},
    u,
    m = {};
  if (o) {
    ...
    let v = f(n.type),
      x = [],
      K = v.length;
    for (let g = 0; g < K; g++) x.push(b(v[g], { structs: s }));
    m = { components: x };
  } else if (n.type in s) (u = "tuple"), (m = { components: s[n.type] });
  else if (we.test(n.type)) u = `${n.type}256`;
  else if (((u = n.type), t?.type !== "struct" && !U(u)))
    throw new E({ type: u });
  if (n.modifier) {
    if (!t?.modifiers?.has?.(n.modifier))
      throw new k({ param: e, type: t?.type, modifier: n.modifier });
    if (P.has(n.modifier) && !Ae(u, !!n.array))
      throw new O({ param: e, type: t?.type, modifier: n.modifier });
  }
  let p = y(y(y({ type: `${u}${n.array ?? ""}` }, i), a), m);
  return B.set(r, p), p;
}
function f(e, t = [], r = "", o = 0) {
  ...
  if (r === "") return t;
  if (o !== 0) throw new _({ current: r, depth: o });
...
function pe(e) {
  ...
  for (let a = 0; a < r; a++) {
    ...
    if (!S(s)) continue;
    let u = X(s);
    if (!u) throw new d({ signature: s, type: "struct" });
    ...
    for (let x = 0; x < v; x++) {
      let g = m[x].trim();
      if (!g) continue;
      let fe = b(g, { type: "struct" });
      p.push(fe);
    }
    if (!p.length) throw new j({ signature: s });
    t[u.name] = p;
  }
  let o = {},
    n = Object.entries(t),
...
function le(e, t, r = new Set()) {
  ...
  for (let i = 0; i < n; i++) {
    let a = e[i];
    if (w.test(a.type)) o.push(a);
    else {
      let u = l(Ee, a.type);
      if (!u?.type) throw new R({ abiParameter: a });
      let { array: m, type: p } = u;
      if (p in t) {
        if (r.has(p)) throw new z({ type: p });
        o.push(
          $(y({}, a), {
            type: `tuple${m ?? ""}`,
      ...
      } else if (U(p)) o.push(a);
      else throw new A({ type: p });
    }
  }
  return o;
}
...
function Ie(e) {
  return e.type === "function"
    ? `function ${e.name}(${h(e.inputs)})${
        e.stateMutability && e.stateMutability !== "nonpayable"
          ? ` ${e.stateMutability}`
          : ""
      }${e.outputs?.length ? ` returns (${h(e.outputs)})` : ""}`
    : e.type === "event"
    ? `event ${e.name}(${h(e.inputs)})`
    : e.type === "error"
    ? `error ${e.name}(${h(e.inputs)})`
    : e.type === "constructor"
    ? `constructor(${h(e.inputs)})${
